...
- [ ] **5.17** **Cierre de fase:** PR, CI verde, merge, hacer un cobro completo en producción — código y pruebas listos en la rama `feat/checkout-money`; falta abrir el PR, que pase CI, y que el usuario apruebe el merge y verifique un cobro real en `fullpetcare-prod` (igual que fases anteriores, el catálogo/datos de negocio no viajan solos con las migraciones — puede hacer falta sincronizar algo a mano)
